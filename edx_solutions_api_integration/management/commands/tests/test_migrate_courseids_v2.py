...
from edx_solutions_api_integration import models as api_models
from progress.models import CourseModuleCompletion
from edx_solutions_api_integration.management.commands import migrate_courseids_v2
...
class MigrateCourseIdsTests(ModuleStoreTestCase):
    ...
    def test_migrate_courseids_v2(self):
        ...
        user = User.objects.create(
            ...
            username='testuser_tmcv2',
            password='testpassword',
            is_active=True
        )
        group = Group.objects.create(name='Test Group')
        group_profile = api_models.GroupProfile.objects.create(group=group)
        course_group = api_models.CourseGroupRelationship.objects.create(
            course_id=self.bad_style_course_id,
            group=group
        )
        course_content_group = api_models.CourseContentGroupRelationship.objects.create(
            course_id=self.bad_style_course_id,
            content_id=self.bad_style_content_id,
            group_profile=group_profile
        )
        course_module_completion = CourseModuleCompletion.objects.create(
            user=user,
            course_id=self.bad_style_course_id,
            content_id=self.bad_style_content_id
        )
        ...
        user2 = User.objects.create(
            email='testuser2@edx.org',
            username='testuser_tmcv2_2',
            password='testpassword2',
            is_active=True
        )
        group2 = Group.objects.create(name='Test Group2')
        group_profile2 = api_models.GroupProfile.objects.create(group=group2)
        course_group2 = api_models.CourseGroupRelationship.objects.create(
            course_id=self.bad_style_course_id2,
            group=group2
        )
        course_content_group2 = api_models.CourseContentGroupRelationship.objects.create(
            course_id=self.bad_style_course_id2,
            content_id=self.bad_style_content_id2,
            group_profile=group_profile2
        )
        course_module_completion2 = CourseModuleCompletion.objects.create(
            user=user2,
            course_id=self.bad_style_course_id2,
            content_id=self.bad_style_content_id2,
            stage=self.bad_style_content_id2
        )
        ...
        updated_course_content_group = api_models.CourseContentGroupRelationship.objects.get(
            id=course_content_group2.id
        )
        ...
        updated_course_module_completion = CourseModuleCompletion.objects.get(id=course_module_completion.id)  # pylint: disable=C0103
        ...
        updated_course_module_completion = CourseModuleCompletion.objects.get(id=course_module_completion2.id)  # pylint: disable=C0103
